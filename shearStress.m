function [C_shear] = shearStress(alpha, b, tStep, rdiv)
...
a(end) = (mat.rho{end} * alpha(b*tStep)) / mat.Q{end}(4,4);
C(end) = (a(end)*rim(end)^4)/8;

b = 1;

for k = 1:length(rim) - 2
  a(end-k) = (mat.rho{end-k} * alpha(b*tStep)) / mat.Q{end}(4,4);
...
for k = 1:length(rim) - 1 
    ...
    rStart = (k-1)*rdiv + 1;
    rEnd = k*rdiv;
    tauArr(1,rStart:rEnd) = mat.Q{b,k}(4,4) * (-(a(k)*dr.^2)./4 + (2*C(k))./dr.^2);
end
