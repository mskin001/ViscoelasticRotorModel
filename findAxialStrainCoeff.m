function [E,C] = findAxialStrainCoeff(sigb)
global mat rim w vari

for b = 1:vari
  syms c1 c2 e0 e1 r
  Fz = 0;
  Mz = 0;
  %% Find constants from boundaries
  [~, kappa, fi] = findMatPropConsts(b,1);
  sInner = -mat.rho{1}*w^2*fi(6)*rim(1)^2 + c1*rim(1)^(kappa-1) + c2*rim(1)^(-kappa-1)...
            + fi(7)*e1*rim(1) + fi(8)*e0 - sigb(1);
  C1 = solve(sInner,c1);
  
  [~, kappa, fi] = findMatPropConsts(b,length(rim)-1);
  sOuter = -mat.rho{end}*w^2*fi(6)*rim(end)^2 + C1*rim(end)^(kappa-1) + c2*rim(end)^(-kappa-1)...
            + fi(7)*e1*rim(end) + fi(8)*e0 - sigb(end);
  C2 = solve(sOuter,c2);
  C1 = subs(C1,c2,C2);

  for k = 1:length(rim)-1
    [Q, kappa, fi] = findMatPropConsts(b,k);
    % Find axial strain constants    
    u = -mat.rho{k}*w^2*fi(1)*r^3 + C1*fi(2)*r^kappa + C2*fi(3)*r^-kappa + fi(4)*e1*r^2 ...
          + fi(5)*e0*r;
    eT = u/r;
    eR = diff(u,'r');
    eZ = e0 + e1*r;
    sigZ = Q(2,1)*eT + Q(2,2)*eZ + Q(2,3)*eR;

    tempF = sigZ*r;
    f = int(tempF,r,rim(k),rim(k+1));
    Fz = Fz + f;

    tempM = sigZ * r^2;
    m = int(tempM,r,rim(k),rim(k+1));
    Mz = Mz + m;
  end
  
  E1 = solve(Fz,e1);
  Mz = subs(Mz,e1,E1);
  E0= solve(Mz,e0);
  E1 = subs(E1,e0,E0);
  E0 = double(E0);
  E1 = double(E1);
  E(b,:) = [E0;E1];
  C1 = subs(C1,{e0,e1},[E0,E1]);
  C1 = double(C1);
  C2 = subs(C2,{e0,e1},[E0,E1]);
  C2 = double(C2);
  C(b,:) = [C1;C2];
  
  [Q, kappa, fi] = findMatPropConsts(b,1);
  InnerStress = -mat.rho{1}*w^2*fi(6)*rim(1)^2 + C(b,1)*rim(1)^(kappa-1) + C(b,2)*rim(1)^(-kappa-1)...
    + fi(7)*E(b,2)*rim(1) + fi(8)*E(b,1) - sigb(1)
  
  [Q, kappa, fi] = findMatPropConsts(b,length(rim)-1);
  OuterStress = -mat.rho{end}*w^2*fi(6)*rim(end)^2 + C(b,1)*rim(end)^(kappa-1) + C(b,2)*rim(end)^(-kappa-1)...
    + fi(7)*E(b,2)*rim(end) + fi(8)*E(b,1) - sigb(2)
end
  
end