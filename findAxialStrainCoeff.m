...
for b = 1:vari
  ...
  sInner = -mat.rho{b,1}*w^2*fi(6)*rim(1)^2 + c1*rim(1)^(kappa-1) + c2*rim(1)^(-kappa-1)...
            + fi(7)*e1*rim(1) + fi(8)*e0 - sigb(1);
  C1 = solve(sInner,c1);

  sOuter = -mat.rho{b,end}*w^2*fi(6)*rim(end)^2 + C1*rim(end)^(kappa-1) + c2*rim(end)^(-kappa-1)...
            + fi(7)*e1*rim(end) + fi(8)*e0 - sigb(2);
  C2 = solve(sOuter,c2);
  C1 = subs(C1,c2,C2);

  % Find axial strain constants
  u = -mat.rho{b,1}*w^2*fi(1)*r^3 + C1*fi(2)*r^kappa + C2*fi(3)*r^-kappa + fi(4)*e1*r^2 ...
        + fi(5)*e0;
  eT = u/r;
  eR = diff(u,r);
  eZ = e0 + e1*r;
